...
async def autopilot():
    ...
    if isinstance(chat.photo, ChatPhotoEmpty):
        photo = await download_file(
            "https://telegra.ph/file/e440505e4a8d6a41fb1c6-9fe6780cac5039ecbc.jpg", "photoyins.jpg"
        )
...
async def autobot():
    ...
    if who.username:
        ...
        username = f"vell{(str(who.id))[5:]}bot"
    bf = "@BotFather"
    ...
    if isdone.startswith("Sorry,"):
        ran = randint(1, 100)
        username = f"vell{str(who.id)[6:]}{ran}bot"
        await bot.send_message(bf, username)
        await asyncio.sleep(1)
        ...
        if nowdone.startswith("Done!"):
            ...
            await bot.send_message(
                BOTLOG_CHATID,
                "**restarting userbot, if there are any problems please contact @bobvell!!**",
            )
        ...
        await bot.send_message(
            BOTLOG_CHATID,
            "**currently restarting userbot, if there are any problems please contact @bobvell!**",
        )
        heroku_var["BOT_TOKEN"] = token
        heroku_var["BOT_USERNAME"] = f"@{username}"
    else:
